...
| 传输 | `rsync` 后端需本机和目标机安装 rsync；`chunk` 后端还需 `find`、`stat`、`sha256sum` |
...
当前 rollback 覆盖：文件备份恢复、包卸载、用户/组删除、systemd/unit disable、Docker Compose down。不能替代整机快照。
...
| 项目 | projects, appdata, home_configs | 项目目录、应用数据、用户配置 |
...
system_baseline 会解析实际配置值（不仅是路径存在性），包括 locale、timezone、sysctl 参数、limits 条目、NTP server/pool、resolv.conf nameserver、nsswitch 查找链、NFS exports、LVM/ZFS/Btrfs 命令输出。
...
| `--operation-id OPS-123/step1` | 关联外部工单系统的操作 ID |
